...
export default class jApi {
    ...
    sets(settings = {}) {
        ...
        if (typeof settings.reloadTimeout === 'number') {
            this.setReloadTimeout(settings.reloadTimeout);
        }
    ...
    call() {
        ...
        if (this.data) {
            if (this.json === true && (this.method === 'POST' || this.method === 'PUT' || this.method === 'PATCH')) {
                data = this.data; // keep as object or string
            } else if (typeof this.data !== 'string') {
                data = param(this.data); // url-encode for form
            } else {
                data = this.data;
            }
        }
    ...
    onResponse() {
        ...
        if (typeof this.callback === 'function') {
            if (false === this.callback(this.response)) {
    ...
    displayMessage(message, type, config) {
        if (this.messenger && typeof this.messenger.add === 'function') {
            this.messenger.add(message, type, config);
            return;
        }
